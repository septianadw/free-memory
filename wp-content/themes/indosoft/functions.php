<?php
/**
 * indosoft functions and definitions.
 *
...
if ( ! function_exists( 'indosoft_setup' ) ) :
/**
...
function indosoft_setup() {
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on indosoft, use a find and replace
	 * to change 'indosoft' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'indosoft', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', 'indosoft' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'indosoft_custom_background_args', array(
		'default-color' => 'ffffff',
		'default-image' => '',
	) ) );
}
endif;
add_action( 'after_setup_theme', 'indosoft_setup' );
 ...
 */
function indosoft_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'indosoft_content_width', 640 );
}
add_action( 'after_setup_theme', 'indosoft_content_width', 0 );
 ...
 */
function indosoft_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'indosoft' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
add_action( 'widgets_init', 'indosoft_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function indosoft_scripts() {
	wp_enqueue_style( 'indosoft-style', get_stylesheet_uri() );

	wp_enqueue_script( 'indosoft-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'indosoft_scripts' );

add_action( 'admin_enqueue_scripts', 'bm_admin_enqueue_scripts');
function bm_admin_enqueue_scripts() {
     wp_enqueue_script('indosoft', get_template_directory_uri() . '/js/sheet-admin.js', array('jquery'));
}
...
/*memanggil file sheet*/
require get_template_directory() . '/template-parts/content-sheet.php';
//require get_template_directory() . '/template-parts/content-page_fact_sheets.php';
...
function bp_scripts() {
    ...
    wp_enqueue_style( 'indosoft_style', get_stylesheet_uri() );
